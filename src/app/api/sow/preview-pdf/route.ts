  ...
  try {
    ...
    try {
      ...
      if (roleInfo && roleInfo.user && roleInfo.user.organizationMemberships && roleInfo.user.organizationMemberships.length > 0) {
        ...
        if (mainLogo) {
          console.log('✅ SOW Preview: Found organization main logo')
        }
        if (secondLogo) {
          console.log('✅ SOW Preview: Found organization secondary logo')
        }
        if (!mainLogo && !secondLogo) {
          console.log('📸 SOW Preview: No organization logos set, will use defaults')
        }
      }
    } catch (error: unknown) {
      console.warn('⚠️ SOW Preview: Could not fetch organization preferences, using default logos:', error)
    }
